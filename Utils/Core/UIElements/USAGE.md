...
UIElements/
  ...
  UIEnums.cs            — Orientation, Justify, Unit, DrawType, …
  SimpleButton.cs       — UIElement subclass with built-in hover/press colors
  ScrollBox.cs          — scrollable viewport container
...
root.customDraw(gameTime);   // clipToBounds elements in a separate scissor pass
```
...
el.setOrientation(Orientation.RELATIVE);  // explicit setPosition() per child
```
...
`UIElement` subclass with `interactive = true`, default hover/press colors, and centred text.

```csharp
var btn = new SimpleButton("Start Game");
btn.setDimensions(160, 40);
btn.onClick = gt => SceneUtility.load<GameScene>();
```
...
Loads a `.html` file from `Content/UI/` and builds a `UIElement` tree. Files must be registered in `Content.mgcb` with `/copy` so they are copied to the output directory.
...
`bindText`, `bindColor`, `bindVisible`, `register` forward to `ctx.bindings`.

---

## BindingContext

Named getters polled each frame; only changed values touch the `UIElement`.
